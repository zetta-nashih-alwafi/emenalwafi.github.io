...
@Component({
  selector: 'ms-transfer-fc-program-candidate-dialog',
  templateUrl: './transfer-fc-program-candidate-dialog.component.html',
  styleUrls: ['./transfer-fc-program-candidate-dialog.component.scss'],
})
export class TransferFcProgramCandidateDialogComponent implements OnInit, OnDestroy {
  private subs = new SubSink();
  assignCampusForm: FormGroup;
  isTitleTrue = false;
  testData;
  studentCount;
  noUsers = false;
  ...
  constructor(
    public dialogRef: MatDialogRef<TransferFcProgramCandidateDialogComponent>,
  ...
  submitAssignMember() {
    if (this.isContinous && !this.assignProgramForm.get('continuous_formation_manager_id').value) {
      ...
      console.log('dev_member', this.selectedMember.name);
  ...
  getTypeFormationDropdown(data?) {
    ...
    this.subs.sink = this.admissionEntrypointService.getAllTypeOfInformationByScholar(this.selectedScholar._id).subscribe(
      (res) => {
        console.log('typeofformation', res);
        if (res) {
          ...
          if (
            ...
            this.data.data[0] &&
            (this.data.data[0].payment === 'pending' ||
              this.data.data[0].payment === 'sepa_pending')
          ) {
            this.formationList = res.filter((ress) => ress.type_of_formation === 'classic');
          } else if (
            this.data &&
            this.data.data &&
            this.data.data.length &&
            ...
            console.log('typeofformation', this.formationList);
          }
          this.initFilter();
        }
        this.isWaitingForResponse = false;
