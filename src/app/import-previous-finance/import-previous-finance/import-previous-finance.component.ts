...
import { ImportPreviousFinanceDialogComponent } from '../import-previous-finance-dialog/import-previous-finance-dialog.component';
import { FinancesService } from 'app/service/finance/finance.service';
import { PermissionService } from 'app/service/permission/permission.service';

@Component({
  selector: 'ms-import-previous-finance',
  templateUrl: './import-previous-finance.component.html',
  styleUrls: ['./import-previous-finance.component.scss'],
})
export class ImportPreviousFinanceComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    this.pageTitleService.setTitle('Platform >> Import of financial N - 1');
  }
  ...
  submitImport() {
    ...
    this.subs.sink = this.financeService.ImportGeneralDashboardFinanceN1(data, this.file).subscribe(
      (resp) => {
        this.isWaitingForResponse = false;
        if (resp) {
          Swal.fire({
          ...
          this.financeService.importOfFinancialN1ValidationStatus = true;
          this.resetImport();
        }
      },
      (err) => {
  ...
  csvTypeSelection() {
    ...
    this.subs.sink = this.dialog
      .open(ImportPreviousFinanceDialogComponent, {
        width: '600px',
        minHeight: '100px',
  ...
  dataIsChanged() {
    JSON.stringify(this.initialForm) === JSON.stringify(this.importForm.value)
      ? (this.financeService.importOfFinancialN1ValidationStatus = true)
      : (this.financeService.importOfFinancialN1ValidationStatus = false);
  }

  canDeactivate(): Promise<boolean> | boolean {
    if (!this.financeService.importOfFinancialN1ValidationStatus) {
