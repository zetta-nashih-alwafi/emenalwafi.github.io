...
@Component({
  selector: 'ms-pre-contract-form-steps-summary',
  templateUrl: './pre-contract-form-steps-summary.html',
  styleUrls: ['./pre-contract-form-steps-summary.scss'],
})
export class PreContractSummaryComponent implements OnInit {
  @Input() userDatas: any;
  @Input() formData: any;
  @Input() stepData: any;
  ...
  ngOnInit() {
    ...
    if (this.formDetail.templateId || (this.formData && this.formData.steps && this.formData.steps.length > 0)) {
      const templateSteps = [];
      ...
      // console.log('_form summary', this.formData.steps);
  ...
  checkDisableForm() {
    const finalValidators = this.formData.final_validators ? this.formData.final_validators.map((validator) => validator._id) : null;
  ...
  downloadPDF() {
    ...
    this.subs.sink = this.formFillingService.generatePDFFcContractProcessStep(this.candidateId, this.stepData._id, lang).subscribe(
      (data) => {
      ...
      (res) => {
        if (res) {
          ...
          if (this.candidateDataFullRate) {
            this.getFullRate(res.scholar_season._id, res.school._id, res.campus._id, res.level._id, res);
          }
        } else {
  ...
  getFullRate(scholar, school, campus, level, resp) {
    this.subs.sink = this.candidateService.GetAllFullRate(scholar, school, campus, level).subscribe(
      (res) => {
        if (res) {
          // console.log('_res', res);
          const fullRate =
            this.candidateDataFullRate.registration_profile_type === 'external' ? res[0].amount_external : res[0].amount_internal;
          ...
          const addtional = this.candidateDataFullRate.selected_payment_plan
          ...
          this.total = this.fullRate - this.dividen;
          if (isNaN(this.total)) {
            this.total = 0;
          }
        } else {
  ...
  getTypeOfFinancement(value) {
    let typeOfFinancement;
    if (value) {
      if (value.length > 0) {
        value.forEach((val) => {
          typeOfFinancement = typeOfFinancement ? typeOfFinancement + ', ' + val : val;
        });
      } else {
        typeOfFinancement = value;
      }
    }
    return typeOfFinancement;
  }
  generateIban(iban) {
    let data = '';
    if (iban) {
