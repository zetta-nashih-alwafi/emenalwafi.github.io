...
@Component({
  selector: 'ms-add-scholar-season-dialog',
  templateUrl: './add-scholar-season-dialog.component.html',
  styleUrls: ['./add-scholar-season-dialog.component.scss'],
  providers: [ParseStringDatePipe, ParseLocalToUtcPipe, ParseUtcToLocalPipe],
})
export class AddScholarSeasonDialogComponent implements OnInit {
  ...
  scholarSeasons: any;
  constructor(
    public dialogRef: MatDialogRef<AddScholarSeasonDialogComponent>,
    ...
    if (this.data && this.data._id) {
      this.subs.sink = this.financeService.UpdateScholarSeason(payload, this.data._id).subscribe(
        ...
        (err) => {
          ...
          this.authService.postErrorLog(err)
          if (
            err['message'] === 'GraphQL error: scholar season name already exist' ||
            err['message'] === 'GraphQL error: Scholar season already in used'
          ) {
            Swal.fire({
