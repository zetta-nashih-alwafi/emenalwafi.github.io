...
export class ImportantboxComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  getImportantMail() {
    ...
    if (this.userId) {
      this.getMainMailBox(pagination, this.sortValue, type, new_mail, filter, this.userId);
    } else {
      this.getMainMailBox(pagination, this.sortValue, type, new_mail, filter);
    }
  }

  getMainMailBox(pagination, sortValue, type, new_mail, filter, userId?) {
    const user = userId ? userId : null;
    this.subs.sink = this.mailboxService.getMainMail(pagination, sortValue, type, new_mail, filter, null, user).subscribe(
      (mailList: any[]) => {
        this.isReset = false;
    ...
    swal
      ...
      .then((result) => {
        ...
        if (result.value) {
          ...
          this.mailboxService.updateMultipleMailRecipient(this.mailSelected, recipient_properties).subscribe(
            (data: any) => {
              swal.fire({
                type: 'info',
    ...
    swal
      ...
      .then((result) => {
        ...
        if (result.value) {
          ...
          const ids = [];
          ids.push(data._id);
          this.mailboxService.updateMultipleMailRecipient(ids, recipient_properties).subscribe(
            (dataa: any) => {
              swal.fire({
                type: 'info',
                title: this.translate.instant('MailBox.MESSAGES.DELMSG'),
