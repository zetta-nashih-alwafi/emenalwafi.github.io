...
@Component({
  selector: 'ms-student-finance-tab',
  templateUrl: './student-finance-tab.component.html',
  styleUrls: ['./student-finance-tab.component.scss'],
  providers: [ParseUtcToLocalPipe, ParseStringDatePipe],
})
export class StudentFinanceTabComponent implements OnInit, OnChanges, OnDestroy {
  @Input() candidateId;
  ...
  getCandidateData() {
    ...
    this.subs.sink = this.candidateService.getCandidateDetails(this.candidateId).subscribe(
      (resp) => {
        if (resp) {
          this.scholarSeasons = this.mapIntakeChannel(resp);
          console.log('==================== {@.@}', this.scholarSeasons);
          this.candidate = _.cloneDeep(resp);
          console.log(this.scholarSeasons)
          this.currentProgram = resp?.intake_channel?.program;
          this.getCurrentProgramTab();
  ...
  getCurrentProgramTab() {
    this.tabIndex = this.scholarSeasons.findIndex(value => this.currentProgram === value?.intake_channel?.program);
  }
