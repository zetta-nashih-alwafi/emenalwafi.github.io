...
@Component({
  selector: 'ms-student-mailbox-inbox-tab',
  templateUrl: './student-mailbox-inbox-tab.component.html',
  styleUrls: ['./student-mailbox-inbox-tab.component.scss'],
})
export class StudentMailboxInboxTabComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  @Input() candidateId;
  @Input() student;
  ...
  getMailbox(reload?) {
    ...
    if (this.candidateId) {
      ...
      this.subs.sink = this.mailboxService
        ...
        .subscribe(
          (mailList: any[]) => {
            console.log('QZWEXSRDTFV', mailList);
            this.isReset = false;
              ...
              mailList.forEach((mail, index) => {
                mail.recipient_properties.forEach((recip) => {
                  recip.recipients.forEach((recipients) => {
                    if (recipients.email === this.student?.email) {
    ...
    if (mailData.recipient_properties) {
      mailData.recipient_properties.forEach((recipient) => {
        recipient.recipients.forEach((mail) => {
          if (recipient.is_read === false && mail.email === this.student?.email) {
        ...
        this.getCountOfCC.forEach((element, indexx) => {
          if (element.rank !== null && element.rank !== undefined) {
            ...
            if (element.rank === 'cc') {
              ...
              element.recipients_email.forEach((mail) => {
                if (element.rank === 'cc' && mail === this.currentUser.email) {
                  // this.viewBcc = true;
                }
              });
            }
  ...
  getTranslationSubjectStatusUpdate(str: string) {
    ...
    if (key) {
      ...
      console.log('_1', key);
      console.log('_1', this.subjectTranslated);
      console.log('_1', this.subjectTranslatedTable);
    }
  }
