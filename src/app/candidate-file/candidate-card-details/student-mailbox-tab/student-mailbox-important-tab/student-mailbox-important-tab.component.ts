...
@Component({
  selector: 'ms-student-mailbox-important-tab',
  templateUrl: './student-mailbox-important-tab.component.html',
  styleUrls: ['./student-mailbox-important-tab.component.scss'],
})
export class StudentMailboxImportantTabComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  @Input() candidateId;
  inbox = false;
  cc = false;
  important = true;
  ...
  getImportantMail() {
    ...
    if (this.candidateId) {
      ...
      this.subs.sink = this.mailboxService.getMainMail(pagination, this.sortValue, type, new_mail, filter, '', this.candidateId).subscribe(
        (mailList: any[]) => {
          this.isReset = false;
  ...
  mailMoveTo(mail_type) {
    ...
    this.viewMessageData = [];
    this.mailboxService.updateMultipleMailRecipient(this.mailSelected, recipient_properties, this.candidateId).subscribe((data: any) => {
      if (!this.isReset) {
        this.getImportantMail();
      }
    });
