...
@Component({
  selector: 'ms-student-mailbox-draft-tab',
  templateUrl: './student-mailbox-draft-tab.component.html',
  styleUrls: ['./student-mailbox-draft-tab.component.scss'],
})
export class StudentMailboxDraftTabComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  @Input() candidateId;
  inbox = false;
  cc = false;
  important = false;
  ...
  OpenDraftGroupMail(MailData) {
    if (MailData._id) {
      ...
      this.subs.sink = this.mailToGroupDialogComponent.afterClosed().subscribe(
        (result) => {
          if (!this.isReset) {
            this.getDraftMail();
          }
