...
@Component({
  selector: 'ms-operation-exported',
  templateUrl: './operation-exported.component.html',
  styleUrls: ['./operation-exported.component.scss'],
  providers: [ParseUtcToLocalPipe],
})
export class OperationExportedComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  accountingDocumentFilter = new UntypedFormControl(null);
  balanceDateFilter = new UntypedFormControl(null);
  ...
  ngOnInit(): void {
    ...
    if (this.scholarFilter && this.scholarFilter.value) {
      const scholarSeason = this.scholarFilter.value !== 'AllF' ? this.scholarFilter.value : null;
      this.getSchool(scholarSeason);
    }
    if (!this.isReset) {
      this.getOperationExported('init');
    }
  ...
  getPermissions(): void {
    this.allowReset = Boolean(this.permissions.operationLinesExportedTabActionResetPermission());
  }
  ...
  ngAfterViewInit(): void {
    this.subs.sink = this.paginator.page
      .pipe(
        ...
        tap(() => {
          if (!this.isReset) {
            this.getOperationExported('after view init');
          }
  ...
  onSort(sort: Sort) {
    ...
    if (this.dataLoaded) {
      ...
      if (!this.isReset) {
        this.getOperationExported('sort');
      }
    }
  }
  ...
  initFilter() {
    ...
    this.subs.sink = this.studentNumberFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('student number filter');
      }
    });
    ...
    this.subs.sink = this.studentNameFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('student name filter');
      }
    });
    ...
    this.subs.sink = this.programFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('program filter');
      }
    });
    ...
    this.subs.sink = this.dateFilter.valueChanges.pipe(debounceTime(400)).subscribe((date) => {
      if (date) {
        ...
        this.clearSelection();
        if (!this.isReset) {
          this.getOperationExported('date filter');
        }
      }
    });
    ...
    this.subs.sink = this.operationNameFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('operation name filter');
      }
    });
    ...
    this.subs.sink = this.payerFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('payer filter');
      }
    });
    ...
    this.subs.sink = this.fluxFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('flux filter');
      }
    });
    ...
    this.subs.sink = this.natureFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      ...
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('nature filter');
      }
    });

    this.subs.sink = this.accountingDocumentFilter.valueChanges.pipe(debounceTime(400)).subscribe((statusSearch) => {
      this.filteredValues.accounting_document = statusSearch ? statusSearch : null;
      this.paginator.firstPage();
      this.clearSelection();
      if (!this.isReset) {
        this.getOperationExported('student name filter');
    ...
    this.subs.sink = this.balanceDateFilter.valueChanges.pipe(debounceTime(400)).subscribe((date) => {
      if (date) {
        const newDate = moment(date).format('DD/MM/YYYY');
        this.filteredValues.balance_date = newDate;
        this.paginator.firstPage();
        this.clearSelection();
        if (!this.isReset) {
          this.getOperationExported('balance date filter');
        }
      }
    });
    // end table filter
  ...
  getCampus() {
    ...
    if (
      ...
      if (
        ...
        this.superFilter.school_ids = this.schoolFilter.value;
        this.currentUser.app_data.school_package.forEach((element) => {
    ...
    if (campuses && campuses.length) {
      campuses.forEach((element, idx) => {
        ...
        campuses[idx]['levels'] = _.uniqBy(levelList, '_id');
      });
    }
  ...
  getSector() {
    ...
    let allCampus = [];
    let allLevel = [];
    if (this.schoolFilter.value && this.schoolFilter.value.includes('AllF') && this.schoolList?.length) {
      allSchool = this.schoolList.map((data) => data._id);
    }
    if (this.campusFilter.value && this.campusFilter.value.includes('AllF') && this.campusList?.length) {
      allCampus = this.campusList.map((data) => data._id);
    }
    if (this.levelFilter.value && this.levelFilter.value.includes('AllF') && this.levelList?.length) {
      allLevel = this.levelList.map((data) => data._id);
    }

    const filter = {
      scholar_season_id:
        this.superFilter.scholar_season_ids && this.superFilter.scholar_season_ids !== 'AllF' ? this.superFilter.scholar_season_ids : null,
      candidate_school_ids:
        this.schoolFilter.value && this.schoolFilter.value.includes('AllF') && allSchool?.length
          ? allSchool
          : this.superFilter.school_ids && !this.superFilter.school_ids.includes('AllF')
          ? this.superFilter.school_ids
          : null,
      campuses:
        this.campusFilter.value && this.campusFilter.value.includes('AllF') && allCampus?.length
          ? allCampus
          : this.superFilter.campus_ids && !this.superFilter.campus_ids.includes('AllF')
          ? this.superFilter.campus_ids
          : null,
      levels:
        this.levelFilter.value && this.levelFilter.value.includes('AllF') && allLevel?.length
          ? allLevel
          : this.superFilter.level_ids && !this.superFilter.level_ids.includes('AllF')
          ? this.superFilter.level_ids
          : null,
    };
  ...
  getDataSpeciality() {
    ...
    if (this.sectorFilter.value && this.sectorFilter.value.includes('AllF') && this.sectorList?.length) {
    ...
    const filter = {
      ...
      levels:
        this.levelFilter.value && this.levelFilter.value.includes('AllF') && allLevel?.length
          ...
          : null,
      sectors:
        this.sectorFilter.value && this.sectorFilter.value.includes('AllF') && allSector?.length
          ? allSector
          : this.superFilter.sector_ids && !this.superFilter.sector_ids.includes('AllF')
          ? this.superFilter.sector_ids
          : null,
    };
  ...
  applySuperFilter() {
    ...
    this.checkSuperFilter();
    if (!this.isReset) {
      this.getOperationExported('apply super filter');
    }
  }
  ...
  applyFromAndToFilter() {
    ...
    this.checkFromAndToFilter();
    if (!this.isReset) {
      this.getOperationExported('apply from and to filter');
    }
  }
  ...
  resetFilter() {
    ...
    this.accountingDocumentFilter.setValue(null, { emitEvent: false });
    this.balanceDateFilter.setValue(null, { emitEvent: false });
    ...
    if (this.isReset) {
      this.getOperationExported('reset');
    }
  }
  ...
  filterBreadcrumbFormat() {
    const filterInfo: FilterBreadCrumbInput[] = [
      ...
      },
      {
        ...
        filterValue: this.filteredValues,
        filterList: null,
        filterRef: this.accountingDocumentFilter,
        displayKey: null,
        savedValue: null,
        isSelectionInput: false,
      },
      {
        ...
        filterValue: this.filteredValues,
        filterList: null,
        filterRef: this.balanceDateFilter,
        displayKey: null,
        savedValue: null,
        isSelectionInput: false,
      },
