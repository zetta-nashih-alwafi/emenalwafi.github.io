...
@Component({
  selector: 'ms-fifth-step-aggrement',
  templateUrl: './fifth-step-aggrement.component.html',
  styleUrls: ['./fifth-step-aggrement.component.scss'],
})
export class FifthStepAggrementComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  identityUpdated() {
    const payloadIntern = {
      internship_creation_step: 'company_manager',
      mentor_id: this.detailForm.get('mentor_selected').value,
    };
    if (
      (this.internshipData.internship_creation_step !== 'pdf' &&
        this.internshipData.internship_creation_step !== 'conditions' &&
        this.internshipData.internship_creation_step !== 'company_manager' &&
        this.internshipData.internship_creation_step !== 'internship') ||
      !this.internshipData.internship_creation_step
    ) {
      payloadIntern.internship_creation_step = 'company_manager';
    } else {
    ...
    this.subs.sink = this.internshipService.updateInternship(this.internshipData._id, payloadIntern).subscribe(
      (resps) => {
        Swal.fire({
          ...
          this.candidateService.setStatusEditModeFive(false);
          this.candidateService.setStatusStepFive(true);
        });
      },
      (err) => {
        Swal.fire({
    ...
    if (this.currSelectedCompanyId && this.typeMentorId) {
      this.subs.sink = this.schoolService.getMentorInternship(this.typeMentorId, this.currSelectedCompanyId).subscribe(
        (resp) => {
          if (resp && resp.length) {
            const mentorSelected = _.cloneDeep(this.detailForm.get('mentor_selected').value);
            if (this.internshipData.mentor_id && this.internshipData.mentor_id._id) {
              if (mentorSelected) {
                this.detailForm.get('mentor_selected').setValue(mentorSelected);
              } else {
                this.detailForm.get('mentor_selected').setValue(this.internshipData.mentor_id._id);
              }
              this.displayMentor(this.internshipData.mentor_id._id);
