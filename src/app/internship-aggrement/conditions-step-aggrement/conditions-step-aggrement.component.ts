...
@Component({
  selector: 'ms-conditions-step-aggrement',
  templateUrl: './conditions-step-aggrement.component.html',
  styleUrls: ['./conditions-step-aggrement.component.scss'],
})
export class ConditionsStepAggrementComponent implements OnInit, AfterViewChecked, OnDestroy {
  @Input() candidateId = '';
  ...
  iniForm() {
    this.detailForm = this.fb.group({
      is_student_already_sign: [false],
      is_mentor_already_sign: [false],
      is_company_relation_member_already_sign: [false],
      company_member_signs: this.fb.array([this.initCRM()]),
    });
  }
  ...
  identityUpdated() {
    ...
    payloadIntern.company_member_signs = payloadIntern.company_member_signs.map((resp) => {
      return {
        is_already_sign: resp.is_already_sign,
        company_member_id: resp.company_member_id._id,
      };
    });
