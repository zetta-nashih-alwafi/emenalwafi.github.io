...
export class DashboardGeneralComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit, AfterViewChecked {
  ...
  getAllGeneralDashboardForChart() {
    ...
    this.subs.sink = this.admissionService.GetAllGeneralDashboardForChart(name).subscribe((resps) => {
              ...
              schoolSelected[0].levels.forEach((temp) => {
                const levelData = temp.campuses.map((temps) => {
                  return {
                    level: temp.name,
                    school: temps.name,
                    total_registered: temps.total_registered,
  ...
  getDataWeeklyProgress() {
    ...
    this.subs.sink = this.admissionService.GetAllGeneralDashboardAdmissions(name).subscribe((resp) => {
      if (resp && resp.length) {
        let allMapping = [];
        resp.forEach((element) => {
          const dataMapping = element.weekly_progresses.map((listProgress) => {
            return {
              ...
              percentage_registration_target_accumulated: listProgress.percentage_registration_target_accumulated,
              registration_accumulated_up_today: listProgress.registration_accumulated_up_today,
              registration_this_week: listProgress.registration_this_week,
              ...
              registration_accumulated_before_week: listProgress.registration_accumulated_before_week,
            };
          });
        ...
        this.dataWeekly = finalResult.filter((temp) => {
          ...
          console.log(dates.isBefore(thisWeek));
          return dates.isBefore(thisWeek);
        });
        this.isLoading = false;
        console.log('finalResult', finalResult, allMapping, this.dataWeekly);
      } else {
        this.isLoading = false;
        this.dataWeekly = [];
      }
