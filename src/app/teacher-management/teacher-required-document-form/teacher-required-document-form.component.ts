...
@Component({
  selector: 'ms-teacher-required-document-form',
  templateUrl: './teacher-required-document-form.component.html',
  styleUrls: ['./teacher-required-document-form.component.scss'],
})
export class TeacherRequiredDocumentFormComponent implements OnInit, OnDestroy {
  ...
  checkDisableForm() {
    ...
    if (this.userData?.entities?.length) {
  ...
  getOneUser() {
    this.isWaitingForResponse = true;
    this.subs.sink = this.formFillingService.getOneUser(this.formDetail?.userId).subscribe(
      (resp) => {
        if (resp) {
  ...
  validateDocument(document, event: MatSlideToggleChange) {
    ...
    document.document_validation_status = event.checked ? 'validated' : 'not_validated';
  ...
  saveUpdatedData(stepDataInput: any) {
    this.isWaitingForResponse = true;
    const payload = this.createPayload(_.cloneDeep(stepDataInput));
    this.formFillingService.createUpdateFormProcessStepAndQuestion(payload).subscribe(
      (resp) => {
        this.isWaitingForResponse = false;
        if (resp) {
          Swal.fire({
  ...
  acceptFormProcessStepV2() {
    ...
    this.isWaitingForResponse = true;
    this.subs.sink = this.formFillingService.acceptFormProcessStepV2(this.stepData?._id, this.formId, payload).subscribe(
      (resp) => {
        this.isWaitingForResponse = false;
        Swal.fire({
          type: 'success',
