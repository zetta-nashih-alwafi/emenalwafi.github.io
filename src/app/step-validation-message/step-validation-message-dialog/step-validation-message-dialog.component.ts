...
@Component({
  selector: 'ms-step-validation-message-dialog-dialog',
  templateUrl: './step-validation-message-dialog.component.html',
  styleUrls: ['./step-validation-message-dialog.component.scss'],
})
export class StepValidationMessageDialogComponent implements OnInit, AfterViewInit {
  ...
  action = 'create';
  constructor(
    public dialogRef: MatDialogRef<StepValidationMessageDialogComponent>,
  ...
  setDialogTitle() {
    ...
    if (this.data) {
      if (this.data.action === 'duplicate') {
        this.dialogTitle = this.translate.instant('REG_STEP_VAL.Duplicate a registration step message');
      }
      if (this.data.action === 'edit') {
        this.dialogTitle = this.translate.instant('REG_STEP_VAL.Update a registration step message');
      }
    } else {
      this.dialogTitle = this.translate.instant('REG_STEP_VAL.Add new registration step message');
    }
  }
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          if (this.data && this.data._id) {
            const _id = _.cloneDeep(this.data._id);
            delete this.data._id;
            service = this.stepValidationMessageService.updateStepValidationMessage(_id, { ...data, gender: null });
          } else {
            service = this.stepValidationMessageService.createStepValidationMessage({ ...data, gender: null });
          }
          service.subscribe((createdValidationStepMessage) => {
            Swal.fire({
              type: 'success',
              ...
              onClose: () => {
                this.dialogRef.close(createdValidationStepMessage);
              },
            });
          });
        }
  ...
  submitStep(data, isPublished = false) {
    if (this.data) {
      const id = this.data._id;
      const payload = _.cloneDeep(data);
      payload.is_published = true;
      payload.gender = 'F';
      this.stepValidationMessageService.editStepValidationMessage(id, payload).subscribe((createdMessage) => {
        Swal.fire({
          type: 'success',
          title: this.translate.instant('Bravo!'),
          allowEscapeKey: true,
