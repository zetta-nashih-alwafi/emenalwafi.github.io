...
export class ConditionDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (
      this.parentData &&
      this.parentData.admission_document &&
      this.parentData.admission_document.document_name &&
      this.parentData.admission_document.s3_file_name
    ) {
      this.uploadDocForm.patchValue(this.parentData.admission_document);
    }
  ...
  submitFlyer() {
    ...
    this.financeService.updateProgram(this.parentData._id, payload).subscribe((resp) => {
      this.isWaitingForResponse = false;
      if (resp) {
