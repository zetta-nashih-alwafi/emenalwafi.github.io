...
@Component({
  selector: 'ms-reconciliation-dialog',
...
export class ReconciliationDialogComponent implements OnInit, OnDestroy {
  ...
  submitVerification() {
    ...
    if (this.selectedPayer) {
      ...
      const studentCivility = this.selectedStudent.civility !== 'neutral' ? this.selectedStudent.civility : '';
      this.data.payment_support = `${this.translate.instant(studentCivility)} ${this.selectedStudent.first_name} ${
        this.selectedStudent.last_name
      }`;
      this.data.from = `${this.translate.instant(studentCivility)} ${this.selectedStudent.first_name} ${this.selectedStudent.last_name}`;
    }
